...
#include "../creativesd/restock_char/char_restock.hpp"
#endif
...
int char_mmo_char_tosql(uint32 char_id, struct mmo_charstatus* p){
	...
	if (
		...
		|| (p->queue_delay != cp->queue_delay)
#endif
#ifdef RANKED_SYSTEM_ENABLE
		|| (p->ranked_points != cp->ranked_points)
#endif
	)
	{	//Save status
...
#ifdef QUEUE_SYSTEM_ENABLE
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, ++col, SQLDT_INT,    &p->queue_delay, 0, NULL, NULL)
#endif
#ifdef RANKED_SYSTEM_ENABLE
	|| SQL_ERROR == SqlStmt_BindColumn(stmt, ++col, SQLDT_FLOAT,  &p->ranked_points, 0, NULL, NULL)
#endif
	)
	{
		SqlStmt_ShowDebug(stmt);
