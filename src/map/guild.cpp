...
int guild_invite(struct map_session_data *sd, struct map_session_data *tsd) {
	...
	// [brAthena] Cell PvP
	if(!battle_config.cell_pvp_guild_invite && (pc_is_on_cellpvp(sd) || pc_is_on_cellpvp(tsd)))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+10)));
		clif_guild_inviteack(sd, 0);
		return 0;
	}
#endif
...
#ifdef CELL_PVP_ENABLE
	// [brAthena] Cell PvP
	if(battle_config.cell_pvp_guild_lock && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+17)));
		return 0;
	}
#endif
