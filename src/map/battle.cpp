...
int64 battle_calc_damage(struct block_list *src,struct block_list *bl,struct Damage *d,int64 damage,uint16 skill_id,uint16 skill_lv)
{
	...
	if( sc && sc->count ) { // SC_* that reduce damage to 0.
		...
		if( (sce = sc->data[SC_LIGHTNINGWALK]) && flag&BF_LONG && rnd()%100 < sce->val1 ) {
			...
			if( unit_movepos(bl, src->x-dx[dir], src->y-dy[dir], 1, 1, 0) )
#else
...
static int battle_calc_attack_skill_ratio(struct Damage wd, struct block_list *src,struct block_list *target,uint16 skill_id,uint16 skill_lv)
{
	...
	switch(skill_id) {
		...
		case NC_MAGMA_ERUPTION: // 'Slam' damage
			...
			break;
		case NC_AXETORNADO:
			skillratio += 200 + 100 * skill_lv + status_get_vit(src);
			RE_LVL_DMOD(100);
	...
	switch (skill_id) {
		...
		case NJ_ISSEN:
			...
			// modified def formula
			{
				...
				md.damage = (int64)sstatus->hp + (atk.damage * 3 * (int64)sstatus->hp * skill_lv) / (int64)sstatus->max_hp;
...
int battle_check_target( struct block_list *src, struct block_list *target,int flag)
{
	...
	if ( s_bl->type == BL_PC ) {
		switch( t_bl->type ) {
			...
			case BL_PC:
#ifdef CELL_PVP_ENABLE
				// Sistema cell_pvp [CarlosHenrq]
				if (pc_targetable_cellpvp((TBL_PC*)s_bl, (TBL_PC*)t_bl))
					return 1;
#endif
...
#ifdef CELL_PVP_ENABLE
							if ( (!mapdata->flag[MF_PVP] || map_is_on_cellpvp(target)) && !mapdata->flag[MF_GVG])
#else
...
#ifdef BG_WARFARE_ENABLE
			if( !map_flag_gvg(m) && md->guardian_data && md->guardian_data->guild_id && !mapdata->flag[MF_BATTLEGROUND] )
#else
	...
	{ //Check rivalry settings.
		...
		if( flag&(BCT_PARTY|BCT_ENEMY) )
		{
			...
			if( s_party && s_party == status_get_party_id(t_bl) && !(mapdata->flag[MF_PVP] && mapdata->flag[MF_PVP_NOPARTY]) && !(mapdata_flag_gvg(mapdata) && mapdata->flag[MF_GVG_NOPARTY]) && (!mapdata->flag[MF_BATTLEGROUND] || sbg_id == tbg_id) )
				state |= BCT_PARTY;
#ifdef BG_WARFARE_ENABLE
			else if (mapdata->flag[MF_BATTLEGROUND] && map_getcell(s_bl->m, s_bl->x, s_bl->y, CELL_CHKNOBATTLEGROUND))
		...
		if( flag&(BCT_GUILD|BCT_ENEMY) )
		{
			...
			if (mapdata->flag[MF_BATTLEGROUND] && (sbg_id || tbg_id))
			{
				if (sbg_id == tbg_id)
					state |= BCT_GUILD;
				else
...
// rAthena
	...
	{ "costumeitem_permission",             &battle_config.costumeitem_permission,       0x00,   0x00,         0x15,        },
#endif
	// [CreativeSD]: Misc Configs
...
void battle_adjust_conf()
{
	...
	if (battle_config.feature_refineui) {
		ShowWarning("conf/battle/creativesd/feature.conf refine UI is enabled but it requires PACKETVER 2016-10-12 or newer, disabling...\n");
		battle_config.feature_refineui = 0;
	}
#else
	// Check if Refine UI is only enabled in scripts
	if( battle_config.feature_refineui == 2 ){
		ShowWarning("conf/battle/creativesd/feature.conf refine UI is enabled in scripts but disabled in general, enabling...\n");
	...
	if (battle_config.feature_petevolution) {
		ShowWarning("conf/battle/creativesd/feature.conf petevolution is enabled but it requires PACKETVER 2014-10-08 or newer, disabling...\n");
		battle_config.feature_petevolution = 0;
	}
	if (battle_config.feature_petevolution) {
		ShowWarning("conf/battle/creativesd/feature.conf pet auto feed is enabled but it requires PACKETVER 2014-10-08 or newer, disabling...\n");
		battle_config.feature_petevolution = 0;
	}
#endif
#endif
