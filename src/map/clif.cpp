...
#include "../creativesd/arealoot/arealoot.hpp"
#endif
...
void clif_selllist(struct map_session_data *sd)
{
	...
	for( i = 0; i < MAX_INVENTORY; i++ )
	{
		if( sd->inventory.u.items_inventory[i].nameid > 0 && sd->inventory_data[i] )
		{
			...
			if( sd->inventory.u.items_inventory[i].card[0]==CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[i].card[2],sd->inventory.u.items_inventory[i].card[3])== battle_config.costumeitem_reserved_id && !(battle_config.costumeitem_permission&0x2) )
				continue;
#endif
...
// [CreativeSD]: Modify clif_item_sub -> struct map_session_data *sd initial argument
...
#ifdef OSHOPS_ENABLE
	// [CreativeSD]: Organized Shops Modify
	if( dstsd->state.vending && (!dstsd->organized_shops.flag || battle_config.organized_shops_ballon) )
#else
	...
	case BL_MOB:
		{
			...
			if (md->bg_id && md->bg_id != sd->bg_id && map_getmapflag(md->bl.m,MF_BATTLEGROUND))
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) =
#if defined(BG_WARFARE_ENABLE) && defined(CELL_PVP_ENABLE)
		(((mapdata->flag[MF_PVP]||map_is_on_cellpvp(bl)||(mapdata->flag[MF_BATTLEGROUND] && map_getcell(bl->m,bl->x,bl->y,CELL_CHKNOBATTLEGROUND)))?1:0)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
#elif defined(BG_WARFARE_ENABLE)
		(((mapdata->flag[MF_PVP]||(mapdata->flag[MF_BATTLEGROUND] && map_getcell(bl->m,bl->x,bl->y,CELL_CHKNOBATTLEGROUND)))?1:0)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
#elif defined(CELL_PVP_ENABLE)
		(((mapdata->flag[MF_PVP]||map_is_on_cellpvp(bl))?1:0)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
#else
		((mapdata->flag[MF_PVP]?1:0)<<0)| // PARTY - Show attack cursor on non-party members (PvP)
#endif
...
void clif_sendegg(struct map_session_data *sd)
{
	...
	// [brAthena]: Cell PvP
	if(battle_config.cell_pvp_pet_egg_return && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(fd, msg_txt(sd, (CELL_PVP_MSG_TXT+15)));
		return;
	}
#endif
...
#ifdef CELL_PVP_ENABLE
	if( (!pc_isinvisible(sd) && pc_is_on_cellpvp(sd)) || mapdata->flag[MF_PVP])
#else
...
#ifdef RANKED_SYSTEM_ENABLE
	// [CreativeSD]: Ranked System
	if( !sd->ranked.disable || (!map_getmapflag(sd->bl.m, MF_TOWN) && sd->ranked.display_timer == INVALID_TIMER) ) {
...
#ifdef EXTENDED_VENDING_ENABLE
/**
 * [CreativeSD/Lilith] Extended Vending system
 **/
int clif_vend(struct map_session_data *sd, int skill_lv) {
	...
	if (battle_config.item_zeny) {
		WFIFOW(fd, d * 2 + 4) = battle_config.item_zeny;
		d++;

	}
	if (battle_config.item_cash) {
		WFIFOW(fd, d * 2 + 4) = battle_config.item_cash;
		d++;
	}
