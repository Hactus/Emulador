...
int unit_walktoxy_sub(struct block_list *bl)
{
	...
	if (!path_search(bl, &wpd, bl->m, bl->x, bl->y, ud->to_x, ud->to_y, ud->state.walk_easy, CELL_CHKNOPASS))
#else
...
#ifdef CELL_PVP_ENABLE
	if (!path_search(bl, &wpd, bl->m, bl->x, bl->y, x, y, flag&1, CELL_CHKNOPASS)) // Count walk path cells
#else
 ...
 */
bool unit_movepos_sub(struct block_list *bl, short dst_x, short dst_y, int easy, bool checkpath, bool skilluse)
{
	short dx,dy;
	uint8 dir;
	struct unit_data        *ud = NULL;
...
#ifdef CELL_PVP_ENABLE
	if( checkpath && (map_getcell(bl->m,dst_x,dst_y,CELL_CHKNOPASS) || !path_search(bl, NULL,bl->m,bl->x,bl->y,dst_x,dst_y,easy,CELL_CHKNOREACH)) )
#else
	...
	if(battle_config.cell_pvp_prevent_movepos
		&& skilluse
		&& map_is_on_cellpvp(bl)
		&& !map_getcell(bl->m, dst_x, dst_y, CELL_CHKPVP))
		return false;
#endif
	...
	if(sd) {
...
#endif
		{
...
#ifdef CELL_PVP_ENABLE
			if( !checkpath && !path_search(pbl, NULL,pbl->m,pbl->x,pbl->y,dst_x,dst_y,0,CELL_CHKNOPASS) )
#else
			...
			if( flag ) {
#ifdef CELL_PVP_ENABLE
				unit_movepos(pbl,sd->bl.x,sd->bl.y, 0, 0, 0);
#else
...
#ifdef CELL_PVP_ENABLE
	return path_search(bl, NULL,bl->m,bl->x,bl->y,x,y,easy,CELL_CHKNOREACH);
#else
...
#ifdef CELL_PVP_ENABLE
	if (!path_search(bl, &wpd,bl->m,bl->x,bl->y,tbl->x-dx,tbl->y-dy,easy,CELL_CHKNOREACH))
#else
