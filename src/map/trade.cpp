...
void trade_traderequest(struct map_session_data *sd, struct map_session_data *target_sd)
{
	...
	if(!battle_config.cell_pvp_can_trade && (pc_is_on_cellpvp(sd) || pc_is_on_cellpvp(target_sd)))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_ENABLE+6)));
		clif_tradestart(sd, 2);
		return;
	}
#endif
...
void trade_tradeadditem(struct map_session_data *sd, short index, short amount)
{
	...
	if( item->card[0]==CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.costumeitem_reserved_id && 
		pc_get_partner(sd) != target_sd && !battle_config.costumeitem_permission&0x01 )
	{
		clif_displaymessage(sd->fd, msg_txt(sd,260));
		clif_tradeitemok(sd, index+2, 1);
		return;
