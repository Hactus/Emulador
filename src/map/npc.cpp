...
#endif
#if defined(STUFF_ITEMS_ENABLE) || defined(SHOP_EXTENDED_ENABLE)
#include "../creativesd/shopextended/shopextended.hpp"
#endif
...
int npc_click(struct map_session_data* sd, struct npc_data* nd)
{
	...
	// [brAthena] Cell PvP
	if(!battle_config.cell_pvp_canrun_npc && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+5)));
		return 1;
	}
#endif
 ...
 * [CreativeSD] Modify extra argument -> int zeny
 */
static int npc_buylist_sub(struct map_session_data* sd, uint16 n, struct s_npc_buy_list *item_list, int zeny, struct npc_data* nd) {
	...
	script_setarray_pc(sd, "@bought_cost", 0, (void*)(intptr_t)zeny, NULL);
...
uint8 npc_buylist(struct map_session_data* sd, uint16 n, struct s_npc_buy_list *item_list) {
	...
	if (nd->master_nd) //Script-based shops.
		return npc_buylist_sub(sd,n,item_list,(int)z,nd->master_nd);
...
static const char* npc_parse_mapflag(char* w1, char* w2, char* w3, char* w4, const char* start, const char* buffer, const char* filepath)
{
	...
	switch( mapflag ){
...
#ifdef RANKED_SYSTEM_ENABLE
		case MF_RANKED_KILL_POINTS_GAIN:
			if (state) {
				float state2;
		...
		case MF_RANKED_KILL_POINTS_DROP:
			if( state )
			{
				float state2;
				if( sscanf(w4, "%f", &state2) == 1 )
