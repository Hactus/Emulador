...
int status_damage(struct block_list *src,struct block_list *target,int64 dhp, int64 dsp, int walkdelay, int flag)
{
	...
	if(src != NULL && src->type == BL_PC && map_is_on_cellpvp(src))
		((TBL_PC*)src)->cell_pvp_canleave_tick = gettick();
	if(target != NULL && target->type == BL_PC && map_is_on_cellpvp(target))
		((TBL_PC*)target)->cell_pvp_canleave_tick = gettick();
#endif
...
int status_calc_mob_(struct mob_data* md, enum e_status_calc_opt opt)
{
	...
	if(flag&4) { // Strengthen Guardians - custom value +10% / lv
		...
		if (!gc
			...
			&& !mapdata->flag[MF_TEST_EMPERIUM]
#endif
		)
			ShowError("status_calc_mob: No castle set at map %s\n", mapdata->name);
#ifdef BG_WARFARE_ENABLE
		else if (mapdata->flag[MF_BATTLEGROUND] && md->mob_id == MOBID_EMPERIUM) {
#ifdef RENEWAL
...
#endif
#ifdef TEST_EMPERIUM_ENABLE
		else if (mapdata->flag[MF_TEST_EMPERIUM] && md->mob_id == MOBID_EMPERIUM) {
#ifdef RENEWAL
			status->max_hp += 50 * 100;
			status->max_sp += 70 * 100;
#else
...
int status_calc_pc_sub(struct map_session_data* sd, enum e_status_calc_opt opt)
{
	...
	for (i = 0; i < EQI_MAX; i++) {
		...
		if (sd->inventory.u.items_inventory[current_equip_item_index].card[0] == CARD0_CREATE && MakeDWord(sd->inventory.u.items_inventory[current_equip_item_index].card[2], sd->inventory.u.items_inventory[current_equip_item_index].card[3]) == battle_config.costumeitem_reserved_id)
			continue;
#endif
...
TIMER_FUNC(status_change_timer){
	...
	case SC_VACUUM_EXTREME:
		if (sce->val4) {
#ifdef CELL_PVP_ENABLE
			if (unit_movepos(bl, sce->val3>>16, sce->val3&0xFFFF, 0, false, false))
#else
