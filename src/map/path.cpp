...
#ifdef CELL_PVP_ENABLE
		// [brAthena] Cell PvP
		if(battle_config.cell_pvp_prevent_blown
			&& map_getcellp(mapdata, x0, y0, CELL_CHKPVP)
			&& !map_getcellp(mapdata, x0 + dx, y0 + dy, CELL_CHKPVP))
			break;
#endif
 ...
 *------------------------------------------*/
// Modify by [CreativeSD/brAthena]
// Change patch_search to patch_search_sub
bool path_search_sub(struct block_list* bl, struct walkpath_data *wpd, int16 m, int16 x0, int16 y0, int16 x1, int16 y1, int flag, cell_chk cell)
{
	...
	if(bl != NULL && bl->type == BL_PC)
	{
		...
		if(battle_config.cell_pvp_delay_leave > 0
			&& map_getcellp(mapdata, x0, y0, CELL_CHKPVP)
			&& !map_getcellp(mapdata, x1, y1, CELL_CHKPVP)
			&& DIFF_TICK(gettick(), sd->cell_pvp_canleave_tick) < battle_config.cell_pvp_delay_leave)
		{
			int diffTime = (int)((battle_config.cell_pvp_delay_leave - DIFF_TICK(gettick(), sd->cell_pvp_canleave_tick)) / 1000);
			char msg[256] = "";

			sprintf(msg, msg_txt(sd, (CELL_PVP_MSG_TXT+4)), diffTime);
