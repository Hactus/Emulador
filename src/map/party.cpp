...
int party_invite(struct map_session_data *sd,struct map_session_data *tsd)
{
	...
	// [brAthena] Cell PvP
	if(!battle_config.cell_pvp_party_invite && (pc_is_on_cellpvp(sd) || pc_is_on_cellpvp(tsd)))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_ENABLE+9)));
		clif_party_invite_reply(sd,tsd->status.name,PARTY_REPLY_REJECTED);
		return 0;
	}
#endif
...
#ifdef CELL_PVP_ENABLE
	// [brAthena] Cell PvP
	if(battle_config.cell_pvp_party_lock && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+16)));
		return 0;
	}
#endif
