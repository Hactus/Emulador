...
int chat_createpcchat(struct map_session_data* sd, const char* title, const char* pass, int limit, bool pub)
{
	...
	// [brAthena]: Cell PvP
	if(!battle_config.cell_pvp_allow_chat && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, CELL_PVP_MSG_TXT));
		return 0;
	}
#endif
...
int chat_joinchat(struct map_session_data* sd, int chatid, const char* pass)
{
	...
	if(!battle_config.cell_pvp_allow_chat && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+1)));
		clif_joinchatfail(sd, 0);
		return 0;
	}
#endif
