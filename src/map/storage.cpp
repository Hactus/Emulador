...
int storage_storageopen(struct map_session_data *sd)
{
	...
	// [CreativeSD]: No Storage
	if( map_getmapflag(sd->bl.m,MF_NOSTORAGE) ) {
		clif_displaymessage(sd->fd, msg_txt(sd,MF_NOSTORAGE_MSG_TXT));
		return 1;
	}
#endif
	...
	if(!battle_config.cell_pvp_can_open_storage && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_ENABLE+8)));
		return 1;
	}
#endif
...
#ifdef MF_NOSTORAGE_ENABLE
	// [CreativeSD]: No Guild Storage
	if( map_getmapflag(sd->bl.m,MF_NOGUILDSTORAGE) ) {
		clif_displaymessage(sd->fd, msg_txt(sd,(MF_NOSTORAGE_MSG_TXT+1)));
