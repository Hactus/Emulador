...
#include "../creativesd/creativesd.hpp"
...
void map_reqnickdb(struct map_session_data * sd, int charid)
{
	...
	if( charid == battle_config.stuff_bg_reserved_id ) {
		clif_solved_charname(sd->fd, charid, msg_txt(sd,STUFF_ITEMS_MSG_TXT));
		return;
	}

	// GvG
	if( charid == battle_config.stuff_gvg_reserved_id ) {
		clif_solved_charname(sd->fd, charid, msg_txt(sd,(STUFF_ITEMS_MSG_TXT+2)));
		return;
	}

	// PvP
	if( charid == battle_config.stuff_pvp_reserved_id ) {
		clif_solved_charname(sd->fd, charid, msg_txt(sd,(STUFF_ITEMS_MSG_TXT+1)));
		return;
	}
#endif
	...
	// Costume System
	if( charid == battle_config.costumeitem_reserved_id ) {
		clif_solved_charname(sd->fd, charid, msg_txt(sd,(COSTUME_SYSTEM_MSG_TXT+2)));
		return;
	}
#endif
...
int map_random_dir(struct block_list *bl, int16 *x, int16 *y)
{
	...
	do {
		...
		(map_getcell(bl->m,xi,yi,CELL_CHKNOPASS) || !path_search(bl,NULL,bl->m,bl->x,bl->y,xi,yi,1,CELL_CHKNOREACH))
#else
...
#ifdef CELL_PVP_ENABLE
/**
 * Verifica se a BL está dentro de uma CELL_PVP.
 *
 * @param bl
 *
 * @return bool Verdadeiro se estiver na celula.
 */
bool map_is_on_cellpvp(struct block_list* bl)
{
	return map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) ? true : false;
}
#endif
...
void do_final(void){
	...
	do_final_creativesd(); // Final CreativeSD
	ShowStatus("Finished.\n");
}
...
int do_init(int argc, char *argv[])
{
	...
	map_do_init_msg();
	do_init_creativesd(); // Init CreativeSD
	do_init_path();
	do_init_atcommand();
	do_init_battle();
	do_init_instance();
