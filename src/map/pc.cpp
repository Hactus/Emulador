...
#include "../creativesd/queue/queue.hpp"
#endif
#ifdef RANKED_SYSTEM_ENABLE
#include "../creativesd/ranked/ranked.hpp"
#endif
#ifdef RESTOCK_SYSTEM_ENABLE
#include "../creativesd/restock/restock.hpp"
#endif
...
int pc_equippoint(struct map_session_data *sd,int n){
	...
	ep = pc_equippoint_sub(sd, sd->inventory_data[n]);

#ifdef COSTUME_SYSTEM_ENABLE
	if( battle_config.costumeitem_reserved_id &&
		sd->inventory.u.items_inventory[n].card[0] == CARD0_CREATE &&
		MakeDWord(sd->inventory.u.items_inventory[n].card[2],sd->inventory.u.items_inventory[n].card[3]) == battle_config.costumeitem_reserved_id)
	{
...
#ifdef COSTUME_SYSTEM_ENABLE
	// [CreativeSD]: Costume System
	if( sd->inventory.u.items_inventory[n].card[0]==CARD0_CREATE &&
	    MakeDWord(sd->inventory.u.items_inventory[n].card[2],sd->inventory.u.items_inventory[n].card[3])== battle_config.costumeitem_reserved_id &&
		(!map_getmapflag(sd->bl.m, MF_COSTUMEON) || map_getmapflag(sd->bl.m, MF_COSTUMEOFF) )
	) {
		if( map_getmapflag(sd->bl.m, MF_COSTUMEOFF) )
			return ITEM_EQUIP_ACK_FAIL;
		if( map_getmapflag(sd->bl.m,MF_PVP) && !(battle_config.costumeitem_maps&0x10) )
			return ITEM_EQUIP_ACK_FAIL;
		if ( map_flag_gvg2(sd->bl.m) && !(battle_config.costumeitem_maps&0x100) )
			return ITEM_EQUIP_ACK_FAIL;
		if( map_getmapflag(sd->bl.m,MF_BATTLEGROUND) && !(battle_config.costumeitem_maps&0x200) )
			return ITEM_EQUIP_ACK_FAIL;
	}
#endif
...
#ifdef CELL_PVP_ENABLE
		if( (map_getmapflag(sd->bl.m, MF_PVP) || pc_is_on_cellpvp(sd)) && !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK) && sd->pvp_timer != INVALID_TIMER )
#else
...
char pc_delitem(struct map_session_data *sd,int n,int amount,int type, short reason, e_log_pick_type log_type)
{
	...
	int restock_nameid = 0, restock_amount = 0;
#endif
...
#ifdef RESTOCK_SYSTEM_ENABLE
	if ( !((!battle_config.restock_castle && map_getmapflag(sd->bl.m, MF_GVG_CASTLE) && agit_flag) || (!battle_config.restock_pvp && map_getmapflag(sd->bl.m,MF_PVP)) || (!battle_config.restock_gvg && map_getmapflag(sd->bl.m,MF_GVG))) )
		{
...
#ifdef CELL_PVP_ENABLE
	// [brAthena] Cell PvP
	if(!battle_config.cell_pvp_drop_item && pc_is_on_cellpvp(sd))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+11)));
		return false;
	}
#endif
...
#ifdef COSTUME_SYSTEM_ENABLE
	// [CreativeSD]: Costume System
	if( item->card[0] == CARD0_CREATE && MakeDWord(item->card[2],item->card[3])==battle_config.costumeitem_reserved_id && !(battle_config.costumeitem_permission&0x08) )
	{
		clif_displaymessage(sd->fd, msg_txt(sd, 264));
		return 1;
	}
...
#ifdef CELL_PVP_ENABLE
	// [brAthena] Cell PvP
	if(!battle_config.cell_pvp_can_teleport
		&& clrtype == CLR_TELEPORT
		&& pc_is_on_cellpvp(sd)
		&& !pc_has_permission(sd, PC_PERM_WARP_ANYWHERE))
	{
		clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+7)));
		return SETPOS_MAPINDEX;
	}
#endif
...
#ifdef CELL_PVP_ENABLE
		if (!battle_config.pvp_exp && (map_getmapflag(sd->bl.m, MF_PVP) || pc_is_on_cellpvp(sd)))  // [MouseJstr]
#else
...
#ifdef CELL_PVP_ENABLE
		|| (battle_config.bone_drop==1 && (map_getmapflag(sd->bl.m, MF_PVP) || pc_is_on_cellpvp(sd)))
#else
...
#ifdef CELL_PVP_ENABLE
	if( (map_getmapflag(sd->bl.m, MF_PVP) || pc_is_on_cellpvp(sd)) && !battle_config.pk_mode && !map_getmapflag(sd->bl.m, MF_PVP_NOCALCRANK) )
#else
...
#ifdef CELL_PVP_ENABLE
	if(pc_is_on_cellpvp(sd2))
	{
		if(!pc_is_on_cellpvp(sd1))
			return 0;
...
#ifdef CELL_PVP_ENABLE
	// Sistema de cell_pvp [CarlosHenrq]
	map_foreachinmap(pc_calc_pvprank_sub,sd->bl.m,BL_PC,sd,&rank_users);

	if(!pc_is_on_cellpvp(sd) && mapdata->flag[MF_PVP])
		rank_users = mapdata->users_pvp;

	if(old!=sd->pvp_rank || sd->pvp_lastusers!=rank_users)
		clif_pvpset(sd,sd->pvp_rank,sd->pvp_lastusers=rank_users,0);
#else
...
#ifdef CELL_PVP_ENABLE
/**
 * Trata os dados dentro de celula pvp.
 *
 * @param sd
 */
void pc_parse_cellpvp(struct map_session_data* sd)
{
	if(sd->cell_pvp_canleave_tick == -1 && pc_is_on_cellpvp(sd))
	{
		sd->pvp_timer = add_timer(gettick() + PVP_CALCRANK_INTERVAL, pc_calc_pvprank_timer, sd->bl.id, 0);
		sd->pvp_rank = 0;
		sd->pvp_lastusers = 0;
		sd->pvp_point = 5;
		sd->pvp_won = 0;
		sd->pvp_lost = 0;
		sd->cell_pvp_canleave_tick = gettick();
		clif_map_property(&sd->bl, MAPPROPERTY_FREEPVPZONE, SELF);
		pc_calc_pvprank(sd);

		if (battle_config.cell_pvp_homunculos_vaporize && hom_vaporize(sd, HOM_ST_ACTIVE))
			clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+12)));

		if(battle_config.cell_pvp_pet_egg_return && sd->pd)
		{
			clif_displaymessage(sd->fd, msg_txt(sd, (CELL_PVP_MSG_TXT+14)));
			pet_menu(sd, 3);
		}

	}
	else if(sd->cell_pvp_canleave_tick != -1 && !pc_is_on_cellpvp(sd))
	{
		sd->cell_pvp_canleave_tick = -1;

		if(!map_getmapflag(sd->bl.m,MF_PVP))
		{
			clif_pvpset(sd, 0, 0, 0);
...
/**
 * Realiza os testes para saber se ambos serão alvos que podem ser acertados dentro do cell_pvp
 *
 * @param s_sd Quem está atacando (source)
 * @param t_sd Quem está recebendo ataque (target)
 */
bool pc_targetable_cellpvp(struct map_session_data* s_sd, struct map_session_data* t_sd)
{
	// Verifica se os dados serão reais e se não é o proprio jogador.
	if(s_sd == NULL || t_sd == NULL || s_sd->bl.id == t_sd->bl.id)
		return false;

	// Verifica se obter o alvo durante a guerra está desabilitado.
	if(battle_config.cell_pvp_disable_on_woe && agit_flag)
		return false;

	// Verifica se o player está em imune a ataques. (@battleignore, @monsterignore)
	if(s_sd->state.monster_ignore || t_sd->state.monster_ignore)
		return false;

	// Verifica se o jogador está em modo invisivel (@hide)
	if(pc_isinvisible(s_sd) || pc_isinvisible(t_sd))
		return false;

	// Verifica se ambos os jogadores estão dentro
	if(!pc_is_on_cellpvp(t_sd)	|| !pc_is_on_cellpvp(s_sd))
		return false;

	// Verifica se os jogadores estão no mesmo grupo e se o cell_pvp
	//  está configurado para dar imunidade ao grupo.
	if(battle_config.cell_pvp_party_immunity
		&& s_sd->status.party_id > 0
		&& t_sd->status.party_id > 0
		&& s_sd->status.party_id == t_sd->status.party_id)
		return false;

	// Membros de um mesmo clã não podem se atacar caso a imunidade da guild esteja ligada.
	if(battle_config.cell_pvp_guild_immunity
		&& s_sd->status.guild_id > 0
		&& t_sd->status.guild_id > 0
		&& s_sd->status.guild_id == t_sd->status.guild_id)
		return false;

	// Testa a diferença de nível dentro da arena e notifica o jogador
	// Que está tentando atacar.
	if(battle_config.cell_pvp_block_level > 0
		&& abs((int)s_sd->status.base_level - (int)t_sd->status.base_level) >= battle_config.cell_pvp_block_level)
	{
		char msg[128] = "";
		sprintf(msg, msg_txt(s_sd, (CELL_PVP_MSG_TXT+18)), t_sd->status.name, t_sd->status.base_level);
		clif_displaymessage(s_sd->fd, msg);
		return false;
	}

	// Retorna verdadeiro, indicando que este jogador é um alvo e pode ser atacado.
	return true;
}

/**
 * Verifica se o jogador não está dentro da celula PVP.
 *
 * @param sd
 *
 * @return bool
 */
bool pc_is_on_cellpvp(struct map_session_data* sd)
{
	return map_is_on_cellpvp(&sd->bl);
}
#endif
